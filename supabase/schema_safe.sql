...
-- Drop the super_admin constraint added by admin_setup.sql so this schema can set its own
...
DROP POLICY IF EXISTS "subscriptions_select_user" ON subscriptions;
...
DROP POLICY IF EXISTS "picture_folders_select_own_org" ON picture_folders;
CREATE POLICY "picture_folders_select_own_org"
  ON picture_folders FOR SELECT
  USING (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "picture_folders_insert_own_org" ON picture_folders;
CREATE POLICY "picture_folders_insert_own_org"
  ON picture_folders FOR INSERT
  WITH CHECK (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "picture_folders_update_own_org" ON picture_folders;
CREATE POLICY "picture_folders_update_own_org"
  ON picture_folders FOR UPDATE
  USING (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "picture_folders_delete_own_org" ON picture_folders;
...
DROP POLICY IF EXISTS "pictures_select_own_org" ON pictures;
CREATE POLICY "pictures_select_own_org"
  ON pictures FOR SELECT
  USING (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "pictures_insert_own_org" ON pictures;
CREATE POLICY "pictures_insert_own_org"
  ON pictures FOR INSERT
  WITH CHECK (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "pictures_update_own_org" ON pictures;
CREATE POLICY "pictures_update_own_org"
  ON pictures FOR UPDATE
  USING (organization_id = get_my_org_id());

DROP POLICY IF EXISTS "pictures_delete_own_org" ON pictures;
...
DROP POLICY IF EXISTS "ai_usage_select_admin" ON ai_usage;
...
DROP POLICY IF EXISTS "discount_codes_select_all" ON discount_codes;
...
DROP TRIGGER IF EXISTS on_org_created_set_trial ON organizations;
