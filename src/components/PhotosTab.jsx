...
function PhotosTab({ project, onUpdateProject, onEditPhoto, onOpenCamera, fileRef, addUploadedPhotos, settings, teamUsers = [], chats = [], onSendPhotoToChat }) {
  ...
  return (
    <div>
      ...
      {/* Floor filter Ã¢ÂÂ only shown when photos have floor data */}
      {floors.length > 0 && (
        <div style={{ display:"flex",alignItems:"center",flexWrap:"wrap",gap:7,marginBottom:10,padding:"8px 14px",background:"var(--surface2)",borderRadius:"var(--radius-sm)",border:"1px solid var(--border)" }}>
          <span style={{ fontSize:11,fontWeight:700,color:"var(--text3)",textTransform:"uppercase",letterSpacing:".05em",whiteSpace:"nowrap" }}>Ã°ÂÂÂ¢ Floor:</span>
          {["all", ...floors].map(f => (
          ...
          {filterFloor !== "all" && (
            <button className="btn btn-ghost btn-sm" style={{ marginLeft:"auto",fontSize:11.5,color:"var(--text3)" }} onClick={() => setFilterFloor("all")}>ÃÂ Clear</button>
          )}
        </div>
      )}
      ...
      <div style={{ display:"flex",alignItems:"center",flexWrap:"wrap",gap:7,marginBottom:14,padding:"10px 14px",background:"var(--surface2)",borderRadius:"var(--radius-sm)",border:"1px solid var(--border)" }}>
        <span style={{ fontSize:11,fontWeight:700,color:"var(--text3)",textTransform:"uppercase",letterSpacing:".05em",whiteSpace:"nowrap" }}>Ã°ÂÂÂ Tags:</span>
        {photoTags.map(tag => (
          <div key={tag} style={{ display:"flex",alignItems:"center",gap:4 }}>
            ...
            <button onClick={() => setEditingTag(tag)} title="Rename" style={{ background:"none",border:"none",cursor:"pointer",color:"var(--text3)",padding:"2px 3px",fontSize:11,lineHeight:1 }}>Ã¢ÂÂ</button>
            <button onClick={() => deleteTag(tag)} title="Delete" style={{ background:"none",border:"none",cursor:"pointer",color:"var(--text3)",padding:"2px 3px",fontSize:11,lineHeight:1 }}>ÃÂ</button>
          </div>
        ))}
        {addingTag ? (
          <input autoFocus className="form-input" style={{ width:110,padding:"2px 8px",fontSize:12,height:26 }}
            placeholder="Tag nameÃ¢ÂÂ¦" value={newTagInput} onChange={e => setNewTagInput(e.target.value)}
            onBlur={addTag}
        ...
        {filterTag !== "all" && (
          <button className="btn btn-ghost btn-sm" style={{ marginLeft:"auto",fontSize:11.5,color:"var(--text3)" }} onClick={() => setFilterTag("all")}>ÃÂ Clear</button>
        )}
      </div>
            ...
            <div key={photo.id} className="photo-card" style={{ outline: selectedPhotoIds.has(photo.id) ? "2.5px solid var(--accent)" : "none", borderRadius: 12 }}>
              ...
              <div className="photo-card-info">
                ...
                <div style={{ fontSize:11.5,color:"var(--text2)",marginBottom:4,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap" }}>
                  {photo.room && <span><span style={{ fontSize:11 }}>Ã°ÂÂÂ</span> {photo.room}</span>}
                  {photo.floor && <><span style={{ color:"var(--border)" }}>ÃÂ·</span><span><span style={{ fontSize:11 }}>Ã°ÂÂÂ¢</span> {photo.floor}</span></>}
                </div>
                {/* Tag chips */}
                {(photo.tags||[]).length > 0 && (
      ...
      {/* Ã¢ÂÂÃ¢ÂÂ Photo Lightbox Viewer Ã¢ÂÂÃ¢ÂÂ */}
      {viewerPhoto && (() => {
        ...
        return (
          ...
          >
            {/* Top bar Ã¢ÂÂ stacks info above buttons on mobile */}
            <div style={{ display:"flex",flexDirection:"column",gap:8,padding:"10px 16px",background:"rgba(0,0,0,.6)",flexShrink:0 }}>
              ...
              <div style={{ minWidth:0 }}>
                ...
                <div style={{ fontSize:11,color:"rgba(255,255,255,.5)",display:"flex",gap:10,flexWrap:"wrap",marginTop:2 }}>
                  {photo.room && <span>Ã°ÂÂÂ {photo.room}{photo.floor?` ÃÂ· ${photo.floor}`:""}</span>}
                  {photo.date && <span>Ã°ÂÂÂ {photo.date}{photo.time?` ${photo.time}`:""}</span>}
                  {photo.gps  && <span>Ã°ÂÂÂ {photo.gps.lat}, {photo.gps.lng}</span>}
                  {(photo.tags||[]).length>0 && <span>Ã°ÂÂÂ· {photo.tags.join(", ")}</span>}
            ...
            {/* Image area Ã¢ÂÂ arrows are siblings on desktop (absolute), row below on mobile */}
            <div className="viewer-img-wrap" style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",position:"relative",minHeight:0,overflow:"hidden" }}>
              ...
              {prev && (
                <button onClick={()=>setViewerPhoto(prev)} title="Previous (Ã¢ÂÂ)" className="viewer-arrow-desktop"
              ...
              {next && (
                <button onClick={()=>setViewerPhoto(next)} title="Next (Ã¢ÂÂ)" className="viewer-arrow-desktop"
              ...
              {/* Mobile arrow row Ã¢ÂÂ shown below image on mobile only */}
              {filtered.length > 1 && (
      ...
      {/* Ã¢ÂÂÃ¢ÂÂ Before & After Pairs Ã¢ÂÂÃ¢ÂÂ */}
          ...
          <div className="modal modal-lg fade-in" style={{ maxWidth:680 }}>
            ...
            <div className="modal-body" style={{ maxHeight:520,overflowY:"auto" }}>
              ...
              <div className="form-group" style={{ marginBottom:16 }}>
                ...
                <input className="form-input" placeholder="e.g. Mold remediated, drywall replacedÃ¢ÂÂ¦" value={baNotes} onChange={e=>setBaNotes(e.target.value)} />
              </div>
...
// Ã¢ÂÂÃ¢ÂÂ Sketch Editor Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
const SKETCH_TOOLS = [
...
export function ProjectDetail({ project, teamUsers = [], chats = [], onBack, onEdit, onOpenCamera, onEditPhoto, onUpdateProject, onOpenReportCreator, onSendVoiceNoteToChat, onSendFileToChat, onSendPhotoToChat, settings, onSettingsChange, orgId, userId }) {
  ...
  return (
    <div className="page fade-in">
        ...
        <div className="project-hero-body">
          ...
          <div className="project-info-grid" style={{ marginTop:16 }}>
            <div className="project-info-box">
              ...
              <div className="project-info-value">{project.clientName || "Ã¢ÂÂ"}</div>
            ...
            <div className="project-info-box">
              ...
              <div className="project-info-value">{project.contractorName || "Ã¢ÂÂ"}</div>
            ...
            <div className="project-info-box">
              ...
              <div className="project-info-value">{project.createdAt ? new Date(project.createdAt).toLocaleDateString("en-US", { month:"short", day:"numeric", year:"numeric" }) : "Ã¢ÂÂ"}</div>
          ...
          {/* Ã¢ÂÂÃ¢ÂÂ Scratch Pad Ã¢ÂÂÃ¢ÂÂ */}
          <div style={{ marginTop:10,padding:"8px 12px",background:"var(--surface2)",borderRadius:8,border:"1px solid var(--border)" }}>
            <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:5 }}>
              ...
              <button
                ...
                {scratchSaved ? "Ã¢ÂÂ Saved" : "Save"}
              </button>
            </div>
            <textarea
              className="form-input"
              rows={2}
              placeholder="Quick notes, measurements, remindersÃ¢ÂÂ¦"
              value={scratchPad}
          ...
          {/* Ã¢ÂÂÃ¢ÂÂ Assigned team members strip Ã¢ÂÂÃ¢ÂÂ */}
          {(() => {
      ...
      {/* Tabs Ã¢ÂÂ horizontally scrollable, compact labels */}
              ...
              <div className="card">
                <div className="card-header" style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  ...
                  {activeStage
                    ...
                    : <span style={{ fontSize:11.5, color:"var(--text3)" }}>No stage set Ã¢ÂÂ click to update</span>
                  }
                </div>
                  ...
                  {project.timelineStage && (
                    <div style={{ marginTop:12, display:"grid", gap:8 }}>
                      <input
                        className="form-input"
                        style={{ fontSize:12.5 }}
                        placeholder={`Internal note for "${activeStage?.label || "this stage"}"Ã¢ÂÂ¦`}
                      ...
                      <input
                        className="form-input"
                        style={{ fontSize:12.5 }}
                        placeholder={`Client portal note for "${activeStage?.label || "this stage"}"Ã¢ÂÂ¦`}
      ...
      {editingSketch !== null && (
        <SketchEditor
          ...
          onSave={(savedSketch, reportId) => {
            ...
            // Ã¢ÂÂÃ¢ÂÂ Fire-and-forget: persist sketch to Supabase Ã¢ÂÂÃ¢ÂÂ
...
// Ã¢ÂÂÃ¢ÂÂ Sketches Tab Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px 14px",flexWrap:"wrap",gap:8 }}>
        ...
        <div style={{ display:"flex",gap:8 }}>
          {selectMode ? (<>
            ...
            {sketches.length > 0 && <button className="btn btn-secondary btn-sm" onClick={() => setSelectMode(true)}>Ã¢ÂÂ Select</button>}
          ...
          <div key={sk.id} style={{ background:"var(--surface)",border:`1px solid ${selectedIds.has(sk.id)?"var(--accent)":"var(--border)"}`,borderRadius:12,overflow:"hidden",cursor:"pointer",position:"relative" }}
            ...
            <div style={{ padding:"10px 12px 12px" }}>
              ...
              <div style={{ fontSize:11.5,color:"var(--text3)",display:"flex",gap:8 }}>
                <span>{sk.date}</span>
                {sk.scale && <span>ÃÂ· {sk.scale}</span>}
              </div>
...
export function TemplatesPage({ projects, onUseTemplate, templates: templatesProp, onTemplatesChange }) {
  ...
  const [useTmpl,     setUseTmpl]     = useState(null);   // template to use Ã¢ÂÂ triggers project picker

  const REPORT_TYPES = ["Insurance","Inspection","Contractor","Damage","Progress","Assessment","Quote","Other"];

  // Map report type Ã¢ÂÂ tag colour class
  const typeTag = type => {
  ...
  // Ã¢ÂÂÃ¢ÂÂ Template edit/create modal Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
        ...
        <div className="modal fade-in modal-lg" style={{ maxWidth:700 }}>
          ...
          <div className="modal-body">
            ...
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Report Type <span style={{ fontWeight:400, color:"var(--text3)" }}>Ã¢ÂÂ sets the tag</span></label>
            ...
            <div className="form-group">
              ...
              <textarea className="form-input form-textarea" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Short description of when to use this templateÃ¢ÂÂ¦" />
            </div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0" }}>
                      ...
                      {isText && isOn && (
                        <button onClick={() => toggleExpand(s)} style={{ background:"none", border:"1px solid var(--border)", borderRadius:6, padding:"4px 12px", fontSize:12, color:"var(--text2)", cursor:"pointer", display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
                          {isOpen ? "Ã¢ÂÂ² Hide" : "Ã¢ÂÂ¼ Edit"}
                        </button>
                      )}
                    </div>
                    {isText && isOn && isOpen && (
                      <div style={{ paddingBottom:14, display:"flex", flexDirection:"column", gap:10 }}>
                        <textarea className="form-input form-textarea" value={secText[s]} onChange={e => setSecText(p => ({ ...p, [s]: e.target.value }))} placeholder={`Default ${s} textÃ¢ÂÂ¦`} style={{ minHeight:100, fontSize:13, resize:"vertical" }} />
                        {isSignOff && (
                          <div>
  ...
  // Ã¢ÂÂÃ¢ÂÂ Project picker modal for "Use" Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
        ...
        <div className="modal fade-in" style={{ maxWidth:480 }}>
          ...
          <div className="modal-body">
            <div style={{ fontSize:13, color:"var(--text2)", marginBottom:14 }}>
              Using <strong style={{ color:"var(--text)" }}>{tmpl.name}</strong> Ã¢ÂÂ select which jobsite to create the report under:
            </div>
            {projects?.length > 0 ? (
