...
export function CameraPage({ project, defaultRoom, onSave, onClose, settings }) {
  ...
  const [gpsLabel,    setGpsLabel]    = useState("LocatingÃ¢ÂÂ¦");
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        await new Promise((resolve) => {
          ...
          vid.play().catch(() => {});
          // Safety timeout Ã¢ÂÂ show stream after 2s even if canplay never fires
  ...
  // Ã¢ÂÂÃ¢ÂÂ Photo capture Ã¢ÂÂÃ¢ÂÂ
  const doSnap = useCallback(async () => {
    ...
    const drawOverlay = (cvs) => {
      ...
      ctx.fillText(`${project?.title || "Jobsite"} Ã¢ÂÂ ${selRoom}  Ã¢ÂÂ¢  ${new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} ${new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:(settings?.timeFormat!=="24hr")})}`, 18, cvs.height - 37);
    ...
    // Ã¢ÂÂÃ¢ÂÂ ImageCapture path (Chrome Android Ã¢ÂÂ supports fillLightMode:'flash') Ã¢ÂÂÃ¢ÂÂ
    const track = streamRef.current?.getVideoTracks()[0];
    if (flashMode === "on" && facing === "environment") {
      if (track && typeof ImageCapture !== "undefined") {
        try {
          const imageCapture = new ImageCapture(track);
          // Call getPhotoCapabilities first Ã¢ÂÂ this warms up the ImageCapture API
    ...
    // Ã¢ÂÂÃ¢ÂÂ Canvas path (no flash / fallback) Ã¢ÂÂÃ¢ÂÂ
    ...
    // Use the video element's rendered bounding rect Ã¢ÂÂ most reliable signal on
  ...
  const acceptPhoto = () => {
    const name = photoName.trim() || `${selRoom} Ã¢ÂÂ ${new Date().toLocaleTimeString()}`;
  ...
  // Ã¢ÂÂÃ¢ÂÂ Video recording Ã¢ÂÂÃ¢ÂÂ
  ...
  const acceptVideo = () => {
    ...
    const name = videoName.trim() || `${selRoom} Ã¢ÂÂ Video ${new Date().toLocaleTimeString()}`;
  ...
  // Desktop without a camera Ã¢ÂÂ show friendly message instead of dark screen
    ...
    <div className="cam-page"><div className="cam-error" style={{ maxWidth:420,margin:"0 auto" }}>
      ...
      <div style={{ fontSize:13,color:"var(--text2)",lineHeight:1.7,textAlign:"left",background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:10,padding:"14px 16px",width:"100%",maxWidth:360 }}>
        ...
        <div style={{ marginBottom:6 }}>
          <strong style={{ color:"var(--text)" }}>Brave:</strong> Click the Ã°ÂÂ¦Â Shields icon Ã¢ÂÂ turn off <em>Device recognition blocking</em>, or click Ã°ÂÂÂ Ã¢ÂÂ Site settings Ã¢ÂÂ Camera Ã¢ÂÂ <strong>Allow</strong>
        </div>
        <div style={{ marginBottom:6 }}>
          <strong style={{ color:"var(--text)" }}>Chrome / Edge:</strong> Click the Ã°ÂÂÂ lock icon in the address bar Ã¢ÂÂ <em>Site settings</em> Ã¢ÂÂ Camera Ã¢ÂÂ <strong>Allow</strong>
        </div>
        <div style={{ marginBottom:6 }}>
          <strong style={{ color:"var(--text)" }}>Safari (iOS):</strong> Settings app Ã¢ÂÂ Safari Ã¢ÂÂ Camera Ã¢ÂÂ Allow
        </div>
        <div>
          <strong style={{ color:"var(--text)" }}>Firefox:</strong> Click the Ã°ÂÂÂ lock icon Ã¢ÂÂ Camera permission Ã¢ÂÂ <strong>Allow</strong>
        </div>
  ...
  return (
    <div className="cam-page">
      ...
      {/* Ã¢ÂÂÃ¢ÂÂ Photo review overlay Ã¢ÂÂÃ¢ÂÂ */}
      {reviewImg && (
          ...
          <div className="review-meta">
            <div style={{ display:"flex",gap:12,marginBottom:12,alignItems:"flex-end" }}>
              <div style={{ flex:1 }}>
                ...
                <input className="form-input" placeholder={`${selRoom} photoÃ¢ÂÂ¦`} value={photoName} onChange={e => setPhotoName(e.target.value)} autoFocus />
              </div>
      ...
      {/* Ã¢ÂÂÃ¢ÂÂ Video review overlay Ã¢ÂÂÃ¢ÂÂ */}
          ...
          <div className="review-meta">
            <div style={{ display:"flex",gap:12,marginBottom:12,alignItems:"flex-end" }}>
              <div style={{ flex:1 }}>
                ...
                <input className="form-input" placeholder={`${selRoom} videoÃ¢ÂÂ¦`} value={videoName} onChange={e => setVideoName(e.target.value)} autoFocus />
              </div>
              <div style={{ minWidth:150 }}>
                <div className="form-label">Room</div>
                <select className="form-input form-select" value={selRoom} onChange={e => setSelRoom(e.target.value)}>
            ...
            <div style={{ fontSize:12,color:"rgba(255,255,255,.6)",marginBottom:10 }}>
              Ã°ÂÂÂ¬ Duration: {fmtTime(recSeconds)} ÃÂ· {gps ? `GPS: ${gps.lat}, ${gps.lng}` : "No GPS"}
            </div>
            <div style={{ display:"flex",gap:10 }}>
          ...
          <div className="review-meta" style={{ maxHeight:"70%",overflowY:"auto" }}>
            ...
            <div style={{ display:"grid",gap:10,marginBottom:10 }}>
              ...
              <div>
                ...
                <textarea className="form-input form-textarea" value={batchNotes} onChange={e => setBatchNotes(e.target.value)} placeholder="Notes to apply to all captured itemsÃ¢ÂÂ¦" style={{ minHeight:88 }} />
              </div>
            </div>
      ...
      <div className="cam-view">
        {/* Spinner overlay Ã¢ÂÂ shown while starting, sits above the video */}
        {camState === "starting" && (
          <div style={{ position:"absolute",inset:0,zIndex:5,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,background:"#000",color:"var(--text2)",fontSize:14 }}>
            ...
            <span>Starting cameraÃ¢ÂÂ¦</span>
          </div>
        )}
        ...
        {/* Rotation lock tip Ã¢ÂÂ shows once on mobile, auto-dismisses after 5s */}
        {showRotateTip && camState === "live" && (
          <div style={{ position:"absolute",bottom:100,left:"50%",transform:"translateX(-50%)",zIndex:30,
            ...
            <button onClick={() => { setShowRotateTip(false); localStorage.setItem("kc_rotate_tip_seen","1"); }}
              style={{ background:"none",border:"none",color:"rgba(255,255,255,0.6)",fontSize:18,cursor:"pointer",padding:"0 0 0 4px",lineHeight:1 }}>ÃÂ</button>
        ...
        <div className="cam-hud-top">
          ...
          <div style={{ display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" }}>
            ...
            <div className="pill" style={{ cursor:"pointer" }} onClick={() => setRoomMenuOpen(o => !o)}>
              <RoomIcon name={selRoom} size={12} stroke="white" /> {selRoom} Ã¢ÂÂ¾
            </div>
          </div>
        </div>
      ...
      <div className="cam-hud-bot">
        ...
        {session.length > 0 && (
          <div style={{ marginBottom:10 }}>
            ...
            <div style={{ fontSize:11,color:"rgba(255,255,255,.55)" }}>
              ...
              {session.filter(s=>s.isVideo).length > 0 && ` ÃÂ· ${session.filter(s=>s.isVideo).length} video${session.filter(s=>s.isVideo).length!==1?"s":""}`} captured
            </div>
          </div>
        )}

        {/* Mode toggle Ã¢ÂÂ Photo / Video */}
        <div style={{ display:"flex",justifyContent:"center",marginBottom:14 }}>
          <div style={{ display:"flex",background:"rgba(0,0,0,.5)",borderRadius:20,padding:3,border:"1px solid rgba(255,255,255,.15)" }}>
            {[{v:"photo",label:"Ã°ÂÂÂ· Photo"},{v:"video",label:"Ã°ÂÂÂ¬ Video"}].map(({v,label})=>(
        ...
        {/* Save session button Ã¢ÂÂ own row so it's never clipped on mobile */}
        {session.length > 0 && (
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:10,alignItems:"center",minWidth:0 }}>
            <div style={{ display:"flex",gap:10,alignItems:"center" }}>
              {facing === "environment" && mode === "photo" && (
                <div
                  ...
                  title={flashMode === "on" ? "Flash on Ã¢ÂÂ tap to turn off" : "Flash off Ã¢ÂÂ tap to turn on"}
                  onClick={toggleTorch}
                  style={flashMode === "on"
            ...
            {mode === "photo" && (
              <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:3 }}>
                <span style={{ fontSize:10,color:"rgba(255,255,255,.6)",fontWeight:600 }}>{zoom.toFixed(1)}ÃÂ</span>
  ...
  return (
    <div className="editor-wrap fade-in">
      ...
      <div className="editor-toolbar">
        <button className="btn btn-sm btn-ghost" onClick={onClose}>Ã¢ÂÂ Back</button>
        ...
        {tool === "crop" && cropRect && Math.abs(cropRect.w) > 4 && Math.abs(cropRect.h) > 4 && (<>
          <div className="tool-sep" />
          <button className="btn btn-sm btn-primary" style={{ fontSize:11.5, padding:"4px 12px" }} onClick={applyCrop}>Ã¢ÂÂ Apply Crop</button>
          <button className="btn btn-sm btn-secondary" style={{ fontSize:11.5, padding:"4px 10px" }} onClick={() => setCropRect(null)}>Ã¢ÂÂ Cancel</button>
        </>)}
        <div className="tool-sep" />
      ...
      {/* Ã¢ÂÂÃ¢ÂÂ Mobile bottom action bar Ã¢ÂÂÃ¢ÂÂ */}
      <div className="editor-actions-mobile" style={{ display:"none",flexDirection:"column",gap:0,background:"var(--surface)",borderTop:"1px solid var(--border)",flexShrink:0 }}>
        {tool === "text" && (
          <div style={{ borderBottom:"1px solid var(--border)" }}>
            <button onClick={() => setMobileControlsOpen(v => !v)}
              ...
              <span>Ã¢ÂÂÃ¯Â¸Â Text Editor {activeTextLayer ? <span style={{ fontSize:11,color:"var(--accent)",fontWeight:400 }}>Ã¢ÂÂ {activeTextLayer.fontSize||42}px</span> : ""}</span>
        ...
        {/* Ã¢ÂÂÃ¢ÂÂ Collapsible brush/color/zoom controls Ã¢ÂÂÃ¢ÂÂ */}
        <div style={{ borderBottom:"1px solid var(--border)" }}>
          <button
            onClick={() => setMobileControlsOpen(v => !v)}
            style={{ width:"100%",background:"none",border:"none",padding:"9px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",color:"var(--text)",fontSize:13,fontWeight:600,cursor:"pointer" }}>
            <span>Ã°ÂÂÂ¨ Brush &amp; Color &nbsp;<span style={{ fontSize:11,color:"var(--text3)",fontWeight:400 }}>{Math.round(zoom*100)}% zoom</span></span>
            <Icon d={mobileControlsOpen ? "M18 15l-6-6-6 6" : "M6 9l6 6 6-6"} size={16} stroke="var(--text3)" />
          </button>
          {mobileControlsOpen && (
            <div style={{ padding:"0 16px 14px",display:"flex",flexDirection:"column",gap:12 }}>
              {/* Zoom */}
              <div>
                <div style={{ fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",color:"var(--text3)",marginBottom:6 }}>Zoom Ã¢ÂÂ {Math.round(zoom*100)}%</div>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <button className="btn btn-sm btn-secondary" style={{ minWidth:36 }} onClick={() => setZoom(z => Math.max(0.25, +(z-0.25).toFixed(2)))}>Ã¢ÂÂ</button>
              ...
              {/* Background/fill color Ã¢ÂÂ hidden for text tool */}
              {tool !== "text" && (
                <div>
              ...
              {tool !== "text" && (
                <div>
                  <div style={{ fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",color:"var(--text3)",marginBottom:6 }}>
                    {tool === "blur" ? "Blur Strength" : "Brush Size"} Ã¢ÂÂ {size}px
                  </div>
                  <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                    <button className="btn btn-sm btn-secondary" style={{ minWidth:34 }} onClick={() => setSize(s => Math.max(10, s-5))}>Ã¢ÂÂ</button>
      ...
      <div className="editor-body">
        <div className="canvas-area">
          ...
          {/* Ã¢ÂÂÃ¢ÂÂ Crop overlay Ã¢ÂÂÃ¢ÂÂ */}
          {tool === "crop" && cropRect && (() => {
            const c = canvasRef.current;
            if (!c) return null;
            const rect = c.getBoundingClientRect();
                ...
                <svg width={W} height={H} style={{ position:"absolute", inset:0 }}>
                  ...
                  {cw > 60 && ch > 30 && (
                    <text x={cx + cw/2} y={cy + ch/2} textAnchor="middle" dominantBaseline="middle"
                      ...
                      {Math.round(Math.abs(cropRect.w))} ÃÂ {Math.round(Math.abs(cropRect.h))}
                    </text>
                  )}
                </svg>
              </div>
        ...
        <div className="editor-side">
          <h4>Zoom Ã¢ÂÂ {Math.round(zoom*100)}%</h4>
          <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:14 }}>
            <button className="btn btn-sm btn-secondary" style={{ minWidth:30 }} onClick={() => setZoom(z => Math.max(0.25, +(z-0.25).toFixed(2)))}>Ã¢ÂÂ</button>
...
// Ã¢ÂÂÃ¢ÂÂ New / Edit Project Modal Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
